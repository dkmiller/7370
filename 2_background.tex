...
Recall the $p$-adic norm $|\cdot|_p:\dQ\to \dR_{\geqslant 0}$, determined 
by the facts that 
\begin{enumerate}
  \item $|a b|_p=|a|_p |b|_p$, 
  \item $|p^r|_p=p^{-r}$, and 
  \item $|m/n|_p=1$ if $p\nmid m,n$. 
\end{enumerate}
The completion of $\dQ$ with respect to this norm is denoted $\dQ_p$. There is 
an obvious field embedding $\dQ\monic \dQ_p$ with dense image. Let 
\[
  \dZ_p=\{x\in \dQ_p:|x|_p\leqslant 1\} .
\]
It turns out that $\dZ_p$ is a compact topological ring which is a complete 
discrete valuation ring. 

\begin{lemma}[Hensel]\label{lem:hensel}
Let $f\in \dZ_p[X]$ satisfy $f(x)\equiv 0\pmod p$ and 
$f'(x_0)\not\equiv 0\pmod p$. Then Newton's method converges ($p$-adically) 
to a solution $x\in \dZ_p$ such that $f(x)=0$ and $x\equiv x_0\pmod p$. 
\end{lemma}
\begin{proof}
Reduce to \'etale schemes over $\spectrum(\dZ_p)$
\end{proof}
...
There is a canonical short exact sequence of Galois groups: 
\[\begin{tikzcd}
  1 \ar[r] 
    & I_p \ar[r] 
    & G_{\dQ_p} \ar[r] 
    ...
    & 1 ,
\end{tikzcd}\]
in which $G_{\dF_p}\simeq \widehat\dZ$ canonically. The group $I_p$, called the 
\emph{inertia group}, can be defined directly by 
\[
  I_p = \{\sigma\in G_{\dQ_p} : \sigma(x)\equiv x\mod\fm_{\overline{\dQ_p}} \text{ for all }x\in \cO_{\overline{\dQ_p}}\} .
\]
Write $\dQ_p^\unram$ for the field $(\overline{\dQ_p})^I$; it turns out that 
$p$ is prime in $\dZ_p^\unram=\cO_{\dQ_p^\unram}$. There is another 
...
\[\begin{tikzcd}
  1 \ar[r] 
    ...
    & 1 .
\end{tikzcd}\]
The group $I^\mathrm{wild}$ is pro-$p$. We don't ``understand'' representations 
$G_\dQ\to \GL_n(\dZ_p)$ largely because 
$G_\dQ\supset G_{\dQ_p}\supset I^\mathrm{wild}$, and we don't understand the 
image of $I^\mathrm{wild}$ in $\GL_n(\dZ_p)$. The field of \emph{$p$-adic Hodge 
theory} seeks to understand this image, among other things. 

Let $E_{/\dQ}$ be an elliptic curve, $\rho_{E,l}:G_\dQ\to \GL_2(\dZ_l)$ the 
Galois representation coming from $\tate_l E$. Say $p$ is a prime of 
\emph{good reduction} for $E$ if a model $y^2=x^3+A x+B$ exists for $E$ over 
$\dQ$ with distinct roots modulo $p$. 

\begin{theorem}[N\'eron-Ogg-Shafarevi\v c]
An elliptic curve $E_{/\dQ}$ has good reduction at $p\ne l$ if and only if 
$\rho_{E,l}(I_p) = 1$. 
\end{theorem}
Since $\rho_{E,l}(I_p)=1$, the representation $\rho_{E,l}|_{G_{\dQ_p}}$ factors 
through $G_{\dF_p} = \widehat\dZ$, which is (canonically) generated by Frobenius 
$\frobenius_p:x\mapsto x^p$. What is $\rho_{E,l}(\frobenius_p)$? It will be a 
diagonalizable matrix conjugate to 
$\begin{pmatrix} \alpha_p \\ & \beta_p \end{pmatrix}$, 
where $\# E(\dF_f) = p^f+1-(\alpha_p^f + \beta_p^f)$. 

What if $E$ has bad reduction at $p$? Then inertia $I_p$ will act nontrivially 
...
\[
  L_p(E,t) = \det\left(1-\frobenius_p\cdot t, (\tate_l E)^{I_p}\right)^{-1} .
\]
We put 
\[
...
[Note: $L$-function of Galois representation\ldots]
...
What about $\rho_{E,p}|_{G_{\dQ_p}}:G_{\dQ_p} \to \GL_2(\dZ_p)$? Even if $E$ 
has good reduction at $p$, this representation can be ramified. If $E$ has good 
reduction, then either good ordinary or good supersingular reduction. In 
good ordinary, we have 
\[
  \rho_p \sim \begin{pmatrix} \varepsilon \chi & \ast \\ & \psi^{-1} \end{pmatrix} ,
\]
where $\psi:G_{\dQ_p}\to \dZ_p^\times$ is unramified and 
$\varepsilon$ is the $p$-adic cyclotomic character, which comes from the 
$p$-adic Tate module of $\Gm$. 

[Cite Ochiai: $l$-independence of trace of monodromy: $\rho_{E,p}|_{G_{\dQ_p}}$ 
``knows'' $L_p(E,s)$ via Fontaine's semistable ring.]

If $E$ has good supersingular reduction at $p$, the this representation is 
irreducible. 
